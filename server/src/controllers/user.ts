...
class UserController {
	...
	async signIn(req: Request, res: Response) {
		try {
			...
			if (!userPassword) throw Error('User password is not defined.');
	
			const isPasswordValid = await bcrypt.compare(req.body.password, userPassword);
			...
			const { password, ...userPL } = userObject;
	...
	async signUp(req: Request, res: Response) {
		try {
			...
			if (!isEmailExisting) {
				...
				if (!userCreated) throw Error("Failed to create user");
	
				const userObject = userCreated.toObject(); // Convert to plain JavaScript object
				const { password, ...userCreatedPL } = userObject;
